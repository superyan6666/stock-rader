...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] 🛡️ 启动 QuantBot 实盘执行网关守护进程 (NUMA/Jemalloc 加速版)..."

# 无限死循环保活机制
while true; do
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] 🟢 正在拉起网关进程: $GATEWAY_SCRIPT"
    
    # 使用 Jemalloc 与 NUMA 节点绑定启动 Python 进程，并将标准输出重定向
    LD_PRELOAD=/usr/lib/aarch64-linux-gnu/libjemalloc.so.2 numactl --cpunodebind=0 --membind=0 python3 $GATEWAY_SCRIPT >> $LOG_FILE 2>&1
