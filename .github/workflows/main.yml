...
on:
  schedule:
    # 🚀 算力集约 1：哨兵改为每天 4 次精华巡航（美东开盘半小时、中午、下午、尾盘）
    - cron: '30 13,15,17,19 * * 1-5'       
    # 🚀 算力集约 2：矩阵每天仅 1 次，锁定美东 16:15 (收盘前45分钟)，日线多空博弈最真实的时刻
    - cron: '15 20 * * 1-5'   
  ...
  run-bot:
    ...
    steps:
        ...
        run: |
          if [[ "${{ github.event_name }}" == "workflow_dispatch" ]]; then
            ...
            if [[ "$UTC_D" == "5" && "$UTC_H" == "23" ]]; then
              MODE="backtest"
            elif [[ "$UTC_H" == "20" && "$UTC_M" -ge 10 && "$UTC_M" -le 25 ]]; then
              MODE="matrix"
            else
              MODE="sentinel"
            fi
          ...
          # 🚀 恢复 All-in-One 极简调用，所有模式均由强大的 quantbot.py 一站式接管
      ...
      - name: 自动提交数据
        ...
        with:
          commit_message: "📝 自动同步量化数据记录与AI模型"
